...
bool LoopMatcher::ms1(KeyFrame* qkf, KeyFrame* tkf, HGraph& h_graph, vector<pair<Detection*, Object*>>& unique_matches, Eigen::Vector2d& drift){
    Eigen::Matrix4d opt_pose = qkf->getPose().cast<double>();
    ...
    const DetectionGroup* dg = dets_unique[0].first->getDetectionGroup();
    Eigen::Matrix3f K = dg->getIntrinsic();
    gtsam::Cal3_S2::shared_ptr K_gtsam(new gtsam::Cal3_S2(K(0, 0), K(1, 1), 0.0, K(0, 2), K(1, 2)));
    gtsam_quadrics::QuadricCamera qcam;
    Eigen::Matrix4d Twc = opt_pose * dg->getSensorPose().cast<double>();
  
    gtsam_quadrics::AlignedBox2 bbox_est = qcam.project(unique_matches[0].second->Q(), gtsam::Pose3(Twc), K_gtsam).bounds();
    gtsam_quadrics::AlignedBox2 bbox_act = unique_matches[0].first->getROI();
    drift = bbox_act.center() - bbox_est.center();

    return true;
}

bool LoopMatcher::ms2(KeyFrame* qkf, KeyFrame* tkf, HGraph& h_graph, const vector<pair<Detection*, Object*>>& unique_matches, const Eigen::Vector2d& drift, Eigen::Matrix4d& opt_pose, double& score, vector<pair<Detection*, Object*>>& match_output){
    vector<Detection*> qkf_dets;
    qkf->getDetections(qkf_dets);
    gtsam::NonlinearFactorGraph base_graph;
    gtsam::Values base_init;
    ...
    for(int r = 0; r < costs.size(); ++r){
        for(int c = 0; c < costs[0].size(); ++ c){
            if(qkf_dets[r]->getClassName() != objects[c]->getClassName()){
                ...
                float dist = (bbox_est.center() + drift - bbox_act.center()).norm();
                costs[r][c] = dist + ((abs(bbox_est.width() - bbox_act.width()) + abs(bbox_est.height() - bbox_act.height()))); // object_uscores[c].second;
            }
        }
    }
    ...
    //For Debug
    // cv::Mat qkf_view_prev;
    // cv::Mat qkf_view_aft;
    // cv::resize(qkf_dets[0]->getDetectionGroup()->view_, qkf_view_prev, cv::Size(0, 0), 5.0, 5.0, cv::INTER_NEAREST);
    // cv::resize(qkf_dets[0]->getDetectionGroup()->view_, qkf_view_aft, cv::Size(0, 0), 5.0, 5.0, cv::INTER_NEAREST);
    //-------Debug---------
...
bool LoopMatcher::match4(KeyFrame* qkf, KeyFrame* tkf, HGraph& h_graph, LoopMatchResult& output){
    unique_lock<mutex> lock(lock_);
    Eigen::Matrix4d opt_pose = qkf->getPose().cast<double>();
    vector<pair<Detection*, Object*>> unique_matches;
    cout<<"MATCH "<<qkf->id()<<" "<<tkf->id()<<endl;
    Eigen::Vector2d drift = Eigen::Vector2d::Zero();
    if(!ms1(qkf, tkf, h_graph, unique_matches, drift)){
        cout<<"failed"<<endl;
        return false;
    }
    ...
    double score = 0.0;
    bool result = ms2(qkf, tkf, h_graph, unique_matches, drift, opt_pose, score, output.object_matches);
    output.drift = tkf->getPose().inverse() * opt_pose.cast<float>();
    output.score = score;
    output.query = qkf->id();
    output.target = tkf->id();
...
bool LoopMatcher::matchStep1(KeyFrame* qkf, KeyFrame* tkf, HGraph& h_graph, Eigen::Matrix4d& opt_pose, vector<pair<Detection*, Object*>>& unique_matches){
    ...
    for(int iter = 0; iter < 1; iter++){
        ...
        // cv::resize(qkf_dets[0]->getDetectionGroup()->view_, qkf_view, cv::Size(0, 0), 5.0, 5.0, cv::INTER_NEAREST);
        // cv::resize(tkf_dets[0]->getDetectionGroup()->view_, tkf_view, cv::Size(0, 0), 5.0, 5.0, cv::INTER_NEAREST);
