...
void SemanticSLAM::registerObjects(KeyFrame* kf){
    ...
    for(auto& det : kf_detections){
        ...
        gtsam_quadrics::BoundingBoxFactor bbf(det->getROI(), K_gtsam, X(kf->id()), O(new_obj->id()), bbox_noise, gtsam_quadrics::BoundingBoxFactor::TRUNCATED);
...
void SemanticSLAM::recordCallback(const std_msgs::BoolConstPtr& msg){
    ...
    string traj_filename = "smslam.txt";
    ...
    string loop_filename= "smslam_loop.txt";
